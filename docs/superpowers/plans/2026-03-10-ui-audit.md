...
- `pnpm typecheck` — passes for `base-ui`; note that `editors` and `ai-ui` have known cross-package type issues and typecheck does not pass for those packages at this time
